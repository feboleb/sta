...
class THWEPOF_Admin_Settings_General extends THWEPOF_Admin_Settings {
	...
	public function reset_to_default() {
		...
		$this->print_notices('Product fields successfully reset.', 'updated', false);
	}
	...
	*-----------------------------------*/
	private function save_or_update_field($section, $action) {
		try {
			...
			$this->print_notices('Your changes were not saved due to an error.', 'error', false);
		}
	}
	...
	private function save_fields($section) {
		try {
			...
			if(empty($f_names)){
				$this->print_notices('Your changes were not saved due to no fields found.', 'error', false);
				return;
			}
