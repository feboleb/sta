...
foreach($file_fields_groups as $file_fields): 
		...
		if(!$disable_stacking_for_variation)
			...
			{
				$order_items[$product_index]['variation_id'] = 0;
			}
		...
		//if( !$display_on_order_detail || !$display_on_thank_you || (isset($file_order_metadata) && !empty($file_order_metadata) && $is_order_completed_status && !$upload_fields_editable_for_completed_orders))
		//if( $current_page == 'order_details' && /* isset($file_order_metadata) && !empty($file_order_metadata) && */ $is_order_completed_status && !$upload_fields_editable_for_completed_orders)
		...
		if(($display_on_order_detail && $current_page == 'order_details') || ($display_on_thank_you && $current_page == 'thank_you') || (($display_on_order_detail || $display_on_thank_you) && $current_page == 'shortcode'))
		{
			...
			if( !array_key_exists($current_order_status, $order_status_to_hide) && (($enable_for === 'always' && $disable_stacking) || $enable_for !== 'always' && (count($selected_categories) > 0 || count($selected_products) > 0 )))
			{
						...
						<div class="wcuf_already_uploaded_data_container"><?php 
							if(!isset($file_fields['message_already_uploaded']))
							{
							...
							else
								{
									...
									$num_total_uploaded_files += count($uploaded_file_data["quantity"]); //each quantity element contains the selected quantity for each file
			...
			<?php  else: //Disable stacking: Upload per product & variant
					...
					<?php 
						...
						if($variation_exists && $product_var_id && ($disable_stacking_for_variation || $product['force_disable_stacking_for_variation']))	
						{
							...
							$product_in_order = $order->get_product_from_item( $product );// apply_filters( 'woocommerce_order_item_product', $order->get_product_from_item( $product ), $product );
