...
class St_Woo_Swatches_Admin extends St_Woo_Swatches_Base {
	...
	public function enqueue_scripts() {
		...
		if( $screen->id == 'shop_order' ) {
		    ...
		    $css = '.st-swatch-preview.st-image-swatch { 
			...
			$css .= '.st-swatch-preview.st-image-swatch span {
				display: inline-block;
				border-radius: 1px;
				width: 100px;
				height: 100px;
		...
		wp_localize_script( $this->plugin_name, 'STI18n', array(
			'wpml_default_lang' => isset( $sitepress['default_language'] ) ? $sitepress['default_language'] : '',
		...
		if ( $plugin == $plugin_file ) {
			...
			$links = array(
				'settings'   => sprintf( '<a href="%1$s">%2$s</a>',
					...
					esc_html__( 'Settings', 'st-woo-swatches' )
				),
				'add-swatch' => sprintf( '<a href="%1$s">%2$s</a>',
					...
					esc_html__( 'Add Swatch', 'st-woo-swatches' )
				),
			);
	...
	public function plugin_row_meta( $actions, $plugin_file ) {
		...
		if( $this->plugin_basename == $plugin_file ) {
			...
			$row_meta = array(
				'docs' => sprintf('<a href="%1$s" target="_blank">%2$s</a>', esc_url( '//sthemes.co/swatches/documentation/' ), esc_html__( 'Documentation','st-woo-swatches') ),				
				'support' => sprintf('<a href="%1$s" target="_blank">%2$s</a>', esc_url( '//codecanyon.net/item/stwooswatches-advanced-variable-product-attributes-swatches-for-woocommerce/21905266/comments' ), esc_html__( 'Premium Support','st-woo-swatches') ),
			);
